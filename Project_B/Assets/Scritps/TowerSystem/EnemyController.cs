...
public class EnemyController : MonoBehaviour
{
    ...
    public float speedaMod = 1.0f;         //속도 선언
    ...
    void Update()
    {
        ...
        if(reacheEnd == false)  // if(!reacheEnd) 도달 이전
        {
            ...
            transform.position =
                Vector3.MoveTowards(transform.position, thePath.points[currentPoint].position, movespeed * Time.deltaTime * speedaMod);

            //Vector3.Distance (A,B) 벡터의 거리 => 거리가 0.01이하일 경우 도착했다고 간주
